...
def test_load_favorited_images_returns_favorite_records(tmp_path):
    favorites = {
    ...
    result = driver._load_favorited_images(tmp_path)
    assert [m["tag"] for m in result] == ["a"]
    assert result[0]["prompt"] == "trentbuckle style, a"


def test_load_favorited_images_returns_empty_without_file(tmp_path):
    assert driver._load_favorited_images(tmp_path) == []
