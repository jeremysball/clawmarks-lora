...
(notes/build_similarity_index.py's precomputed nearest-neighbor lists), and a pick button that
marks an image as a human-judged winner via notes/curation_server.py's API. Meant for actually
scrolling through the whole batch by eye and guiding the next search run, unlike
notes/uncanny_sweep/gallery.html's binned descriptor grid (capped at 12 thumbnails per bin, built
for the faithfulness/novelty map shape, not for browsing or picking).

Run after scored_manifest.json (and, for "show similar," similarity.json) exist:
    python3 notes/build_scan_gallery.py
Then open via notes/curation_server.py (NOT plain http.server, which can't accept picks):
    python3 notes/curation_server.py
    http://100.75.221.31:8420/scan.html
"""
